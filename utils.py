...
def random_generator (batch_size, z_dim, T_mb, max_seq_len):
  ...
  for i in range(batch_size):
    ...
    temp[:max_seq_len,:] = temp_Z
    Z_mb.append(temp_Z)
  return np.array(Z_mb, dtype=np.float32)
...
def batch_generator(data, time, batch_size):
  ...
  T_mb = np.array([time[i] for i in train_idx], dtype=np.float32)
  
  return X_mb, T_mb

def npy_to_pkl(dir):
  scenarios = scenarios = ['osc_free_30',
 #'osc_free_fast_30',
 'rand_free_30',
 'osc_tip_30',
 'rand_tip_30']
 ...
 #'rand_rand_60']
  ...
  for s in scenarios:
      data = np.load(f'output/{dir}/{s}/synthetic_data.npy')
      print(s, data.shape)
      with open(f'output/{dir}/{s}/{s}.pkl', 'wb') as f:
          pickle.dump(data, f)