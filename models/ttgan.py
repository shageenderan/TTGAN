# TransformerTimeGAN
"""
Base TimeGAN class implemented accordingly with:
...
class TimeGAN(GAN):
    def __init__(self, model_parameters, hidden_dim, seq_len, n_features, gamma):
    ...
    def define_gan(self):
        self.generator_aux=Generator(self.hidden_dim).build(input_shape=(self.seq_len, self.n_features))
        self.supervisor=Supervisor(self.hidden_dim).build(input_shape=(self.seq_len, self.hidden_dim))
        self.discriminator=Discriminator(self.hidden_dim).build(input_shape=(self.seq_len, self.hidden_dim))
        ...
        Z = Input(shape=[self.seq_len, self.n_features], batch_size=self.batch_size, name='RandomNoise')
    ...
    def _generate_noise(self):
        while True:
            yield np.random.uniform(low=0, high=1, size=(self.seq_len, self.n_features))
    ...
    def train(self, data, train_steps):
        ...
        dataset = self.get_batch_data(data, n_windows=len(data))
        noise = self.get_batch_noise()

        _dataset = self.get_batch_data(data, n_windows=len(data))
        _noise = self.get_batch_noise()
